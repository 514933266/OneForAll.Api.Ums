...
namespace Ums.HttpService
{
    ...
    /// 企业微信
    /// </summary>
    public class WechatQyHttpService : BaseHttpService, IWechatQyRobotHttpService
    {
        private readonly HttpServiceConfig _config;
        public WechatQyHttpService(
        ...
        /// <returns></returns>
        public async Task<BaseMessage> SendTextAsync(WechatQyRobotTextRequest request, string url)
        {
            ...
            if (client != null)
            {
                ...
                var result = await res.Content.ReadAsAsync<WechatQyRobotResponse>();
                if (result.ErrCode == "0")
                {
        ...
        /// <param name="url">机器人地址</param>
        /// <returns></returns>
        public async Task<BaseMessage> SendMarkdownAsync(WechatQyRobotMarkdownRequest request, string url)
        {
            var msg = new BaseMessage();
            var client = new HttpClient();
            if (client != null)
